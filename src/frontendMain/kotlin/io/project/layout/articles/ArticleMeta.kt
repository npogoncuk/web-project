...
fun Container.articleMeta(article: Article, state: GLogState) {
        ...
        if (article.author?.username != state.user?.username) {
            if (article.author?.following == true) {
                button(
                    ...
                    onClick {
                        GLogManager.toggleProfileFollow(article.author)
                    }
                }
            } else {
                button(
            ...
            if (article.favorited) {
                button("Unfavorite Post ", "ion-heart", separator = "&nbsp; ") {
                    ...
                    onClick {
                        GLogManager.toggleFavoriteArticle(article)
                    }
                }
            } else {
            ...
            button("Delete Article", "ion-trash-a", ButtonStyle.OUTLINEDANGER) {
                ...
                GLogManager.deleteArticle(article.slug!!)
            }
        }
    }
}
